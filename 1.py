...
parts_df = tweets_all.withColumn("parts", f.explode(
    f.split(f.col('transform_text(all)'), ' '))).select("parts")
...
# top_15_hashtags_df.write.csv('./top_15_hashtags.csv', header=True)
top_15_hashtags_df.write.csv(
    'hdfs://compute1:9000/user/danilo-p/top_15_hashtags.csv', header=True)
